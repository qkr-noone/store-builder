<template>
  ...
  <div data-attr="手机端" class="phone">
    <div class="phone_box" :style="'height:'+ autoHeight + 'px;'">
      ...
      <iframe ref="iframe" :src="'/#/gstore/'+curIframePage+'?setmobiletag=1&storeId='+storeId" scrolling="no" frameborder="0"></iframe>
    </div>
            ...
            <div class="set_con_banner_info set_con_banner_info_two">
              ...
              <div class="pane-box" style="border-bottom: 1px solid #663399;">
                <ul class="pane_limit_height pane_limit_goods">
                  <li v-for="list in storeGoods.records" :key="list.id">
                    <span><input type="checkbox" :value="list" v-model="pickList" @change="goodsChange($event)"/></span>
                    <span class="pane_good_img">
                      <img :src="list.imagesCover"> <!--  @click="goPage(WEBSITE,'/detail',{goodsId: list.id})" -->
                      <a
                        v-if="list.threeId"
                        target="_blank"
                        class="t3D_play"
                    ...
                    <span>
                      <span>{{list.priceShow}}{{list.priceUnit}}</span>
                    </span>
            ...
            <div class="set_con_banner_info set_con_banner_info_two">
              ...
              <div class="pane-box" style="border-bottom: 1px solid #663399;">
                <ul class="pane_limit_height video_pane_limit_height" v-if="storeRes3D.total">
                  <li class="video_li_pane_limit_height" v-for="list in storeRes3D.records" :key="list.id">
                    <div class="video_pane_box tD_item_box">
                      <a class="video_pane_box_img_box">
                        <img class="video_pane_box_img" style="display: block; background-color: #000;" :src="list.image">
                      </a>
                    </div>
  ...
  methods: {
    ...
    save () {
      if (this.currentComponent.isSign) {
        this.isLoading = true
        this.APIG.saveAppSign(this.currentComponent).then(res => {
        ...
        if (this.pageIndex === 3) {
          this.isLoading = true
          this.APIG.saveAppWindow(this.currentComponent.dataList).then(res => {
          ...
          this.APIG.saveAppBanner(this.currentComponent.dataList, this.markDeleteImgId.join(',')).then(res => {
        ...
        this.APIG.saveAppProduct(goodObj).then(res => {
        ...
        this.isLoading = true
        this.APIG.saveAppVideo(this.currentComponent).then(res => {
    ...
    // 商品列表
    goodsData () {
      this.APIG.getGoodsList({ titleOrArticleNumber: this.searchGoodsName, page: this.page, rows: this.rows, goodsGroupId: this.currentCate }).then(res => {
    ...
    // 获取3D 商品列表
    get3DData () {
      this.APIG.get3DList({ page: this.threeD.page, rows: this.threeD.rows, key: this.threeD.goodsName }).then(res => {
